 ...
 * Results are cached in the Next.js Data Cache to eliminate database latency on API calls (like RAG search).
 */
export const getAllWisdom = unstable_cache(
  async (): Promise<Wisdom[]> => fetchAllWisdom(),
  ["all-wisdom-cards"],
  {
    revalidate: 300, // 5 minutes, invalidated immediately via revalidateTag("wisdom-cards") on publish
    tags: ["wisdom-cards"],
  }
);
      ...
      if (!error && data?.length) {
        ...
        return data.map((row: any) => {
          ...
          return {
            ...
            sidebar_banner: row.sidebar_banner || undefined,
            sidebar_focal_point: row.sidebar_focal_point || undefined,
            tags: tags,
