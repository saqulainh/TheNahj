...
export const wisdomArticleSchema = z.object({
  ...
  hero_focal_point: z.preprocess((v: any) => (!v || typeof v !== 'object' || Number.isNaN(v.x) || Number.isNaN(v.y) ? null : v), z.object({ x: z.number().min(0).max(100), y: z.number().min(0).max(100) })).optional().nullable(),
  featured_focal_point: z.preprocess((v: any) => (!v || typeof v !== 'object' || Number.isNaN(v.x) || Number.isNaN(v.y) ? null : v), z.object({ x: z.number().min(0).max(100), y: z.number().min(0).max(100) })).optional().nullable(),
  sidebar_focal_point: z.preprocess((v: any) => (!v || typeof v !== 'object' || Number.isNaN(v.x) || Number.isNaN(v.y) ? null : v), z.object({ x: z.number().min(0).max(100), y: z.number().min(0).max(100) })).optional().nullable(),
