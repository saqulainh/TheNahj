...
import { getThemesForSection, getTopicsForSection, normalizeSection, normalizeThemeForSection } from "@/lib/taxonomy";
...
export async function GET(request: Request) {
  ...
  if (theme) {
    const resolved = normalizeThemeForSection(section, theme);
    if (!resolved) return NextResponse.json({ tags: [] });
    return NextResponse.json({ tags: getTopicsForSection(section, resolved) });
  }
