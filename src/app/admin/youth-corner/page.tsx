...
import { Users, Sparkles, Search, Calendar, Plus, Trash2 } from "lucide-react";
...
const initialForm = {
  ...
  detox_checklist: [] as string[],
  publish_date: "",
  meta_title: "",
  meta_description: "",
};

export default function YouthCornerCMSPage() {
  ...
  const addStep = () => {
    if (stepInput.trim() && !form.detox_checklist.includes(stepInput.trim())) {
      update("detox_checklist", [...form.detox_checklist, stepInput.trim()]);
      setStepInput("");
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setStatus("Publishing youth corner asset...");
    setTimeout(() => {
      setStatus("Youth corner asset published successfully!");
      setForm(initialForm);
    }, 700);
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3 border-b border-border pb-5">
        ...
        <div>
          <h1 className="text-3xl font-medium tracking-tight text-foreground">Youth Corner CMS</h1>
          <p className="mt-1 text-sm text-muted">Create social shield guides, vanity checks, and peer connection materials.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-8">
          ...
          <div className="space-y-6 rounded-xl border border-border bg-surface p-6">
            ...
            <label className="block">
              ...
              <input
                ...
                required
                placeholder="e.g. Shielding from peer vanity"
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-border bg-surface p-6 space-y-4">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gold-muted">Social Shield Steps</h3>
            <div className="flex gap-2">
              <input
                type="text"
                value={stepInput}
                onChange={(e) => setStepInput(e.target.value)}
                placeholder="e.g. Mute distracting social notifications"
            ...
            <div className="flex flex-col gap-2">
              {form.detox_checklist.map((step, idx) => (
                <div key={idx} className="flex items-center justify-between rounded-lg bg-background p-2.5 text-xs text-muted border border-border">
                  ...
                  <Trash2
                    ...
                    onClick={() => update("detox_checklist", form.detox_checklist.filter((_, i) => i !== idx))}
                  />
                </div>
              ))}
            </div>
          ...
          {/* Reflection Prompts */}
          <div className="rounded-xl border border-border bg-surface p-6 space-y-4">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gold-muted">Youth Reflection Questions</h3>
            <div className="flex gap-2">
              <input
                type="text"
                value={questionInput}
                onChange={(e) => setQuestionInput(e.target.value)}
                placeholder="e.g. Do I seek validation from likes or inner character?"
                className="flex-1 rounded-xl border border-border bg-background px-3 py-2 text-sm focus:border-gold/40 focus:outline-none"
              />
              <button
                type="button"
